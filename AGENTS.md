...
8. **Prioritize conceptual consolidation.** Prefer a single concept note (e.g., `[[logos-gabilondo]]`) that synthesizes multiple sources rather than fragmented notes. The goal is to provide the LLM with a "big picture" view that facilitates cross-cutting and coherent responses.
